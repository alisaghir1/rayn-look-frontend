...
const Adminproductcard = ({data, onDelete}) => {
    ...
    const handleImageDelete = async () => {
        try {
            ...
            const response = await axios.delete(`http://localhost:8080/Product/${newData._id}/${deleteIndex}`);
            if (response.status !== 200) {
                throw new Error(`HTTP error! status: ${response.status}`);
            }
    ...
    const handleImageAdd = async (event) => {
        try {
            ...
            const response = await axios.post(`http://localhost:8080/Product/${newData._id}`,formData);
            const updatedImages = [...newData.Image];
            setNewData({ ...newData, Image: updatedImages });
    ...
    const handleSubmit = async (event) => {
        try {
            ...
            const response = await axios.patch(`http://localhost:8080/Product/${newData._id}`, requestData, {
                headers: {
    ...
    return(
        <>
                ...
                <Modal.Body>
                    <Form >
                        <Form.Group controlId="formName">
                            <Carousel controls={true} indicators={false} interval={null} onSelect={(selectedIndex) => {setDeleteIndex(selectedIndex); console.log(selectedIndex, newData._id);}}>
                                {newData.Image.map((image, index) => (
                                    <Carousel.Item key={index}>
                            ...
                            <div className="IMG-BUTTON">
                                <Button variant="danger" className="DeleteImageButton" onClick={ () => {handleImageDelete(); onDelete();}} >
                                    Delete Image
                                </Button>
                                <Button variant="success" className="DeleteImageButton" onClick= { () => {setImageForm(true); setShow(false)} } >
                                    Add Image
                                </Button>
                            </div>
                        </Form.Group>
            ...
            <Modal show={ImageForm} onHide={() => {setImageForm(false);  setShow(true)}}>
            <Modal.Header closeButton>
                ...
                <Modal.Body>
                    <Form >
                        ...
                        <Button variant="primary mt-3" className="SubmitButton" type="submit" onClick= {  (event) => {handleImageAdd(event); setCreationImages("")}} >
                            Submit
                        </Button>
                    </Form>
                </Modal.Body>
